...
public class MovieDetailsEngine {
	public static MovieDetailsObj parseMovieDetails(String json) {
		...
		try {
			...
			movieObject.setTitle(movieDtls.getString("title").replaceAll(Replace, ReplaceTo));
			movieObject.setRating(movieDtls.getString("rating").replaceAll(Replace, ReplaceTo));
			movieObject.setImage(movieDtls.getString("image").replaceAll(Replace, ReplaceTo));
			movieObject.setGenres(movieDtls.getString("Genre").replaceAll(Replace, ReplaceTo));
			movieObject.setSubject(movieDtls.getString("subject").replaceAll(Replace, ReplaceTo));
			movieObject.setActors(movieDtls.getString("Actor").replaceAll(Replace, ReplaceTo));
			movieObject.setDirectors(movieDtls.getString("Director").replaceAll(Replace, ReplaceTo));
			movieObject.setWriters(movieDtls.getString("Writer").replaceAll(Replace, ReplaceTo));
			movieObject.setDuration(movieDtls.getString("duration").replaceAll(Replace, ReplaceTo));
			movieObject.setOverview(movieDtls.getString("overview").replaceAll(Replace, ReplaceTo));
			
			JSONArray jsonLocnArr = new JSONArray(
					movieDtls.getString("filmLocations"));
			JSONObject filmLocObj = (JSONObject) jsonLocnArr.get(0);
			movieObject.setLanguage(MenuData.language.get(filmLocObj
					.getInt("languageId")));
